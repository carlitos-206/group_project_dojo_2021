...
def edit(request, group_id):
    if 'user_id' in request.session:
        ...
        if {tableXXX}.objects.filter(id=group_id, owner=this_owner):
...
def update(request, group_id):
    errors = Group.objects.XXX_validator(request.POST)
        if len(errors) > 0 :
            for key, value in errors.items():
            messages.error(request, value)
        return redirect('edit/{group_id}')
    to_update = Group.objects.git(id=group_id)
    to_update.group_name = request.POST['group_name']
    to_update.group_genre = request.POST['group_genre']
